...
import { api } from "../lib/api";
import type { TimelineEntry, EntryType, SourceApi } from "../types/TimelineEntry";

// Helper to ensure the string matches one of our allowed EntryTypes
const normEntryType = (s?: string | null): EntryType => {
  if (!s) return "Note";
  const t = s.trim();
  // We cast strictly to ensure TS is happy, assuming backend sends valid data
  // or we default to "Note"
  if (["Achievement", "Activity", "Milestone", "Memory", "Note"].includes(t)) {
...
const normSource = (s?: string | null): SourceApi => {
  if (!s || !s.trim()) return "manual";
  return s.trim() as SourceApi;
};
...
function fromApi(e: ApiEntry): TimelineEntry {
  return {
    ...
    description: e.description ?? undefined,
    eventDate: e.eventDate,
    entryType: normEntryType(e.entryType),
    category: e.category ?? undefined,
    imageUrl: undefined,        
    fileAttachment: e.fileAttachment ?? undefined,
    fileName: e.fileName ?? undefined,
    fileType: e.fileType ?? undefined,
    sourceApi: normSource(e.sourceApi),
    externalId: undefined,
    metadata: e.metadata ?? undefined,
    createdAt: e.createdAt ?? undefined,
    updatedAt: undefined,
...
export const timelineApiService = {
  async list(params?: {
    ...
    // Fix for Date errors: explicitly check for string existence before new Date()
    if (params?.from) {
      const fromDate = new Date(params.from);
      items = items.filter(e => new Date(e.eventDate) >= fromDate);
    }
    if (params?.to) {
      const toDate = new Date(params.to);
      items = items.filter(e => new Date(e.eventDate) <= toDate);
    }
